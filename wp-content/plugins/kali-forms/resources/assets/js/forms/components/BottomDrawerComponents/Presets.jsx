...
const Presets = props => {
	...
	const [predefinedOptions, setPredefinedOptions] = React.useState(KaliFormsObject.predefinedOptions);
	...
	return (
		<React.Fragment>
			<Grid container>
				<Grid item xs={3}>
					<Typography variant={'h3'}>{KaliFormsObject.translations.bottomDrawer.presets}</Typography>
					<p>
						{KaliFormsObject.translations.bottomDrawer.presetInfo}
					</p>
				</Grid>
				<Grid item xs={3}>
				...
				<Grid item xs={categoryHasSubcategory() ? 3 : 6} className={classes.differentBg}>
					...
					<If condition={!categoryHasSubcategory() && category !== ''}>
						<Button onClick={e => updateField()} className={classes.importButton}>{KaliFormsObject.translations.bottomDrawer.importPresets}</Button>
					</If>
				</Grid>
				<If condition={categoryHasSubcategory()}>
